...
Exposes 91 tools covering real-time monitoring, host and service configuration, downtime and acknowledgement management, infrastructure, users, and notifications. Integrates with any MCP-compatible AI client such as Claude Code or Claude Desktop.

## Features

- **91 tools** across 11 categories: monitoring, operations, downtimes, acknowledgements, host config, service config, infrastructure, users, notifications, platform status, and connection testing
...
| `MCP_READ_ONLY`             | No       | `false`     | Register the mutating tools but refuse them: any tool that writes to Centreon returns a read-only error and performs no action. Read tools are unaffected. |
...
> **Note on host URLs:** a URL that names no hostname is rejected everywhere a host is validated, because it is a typo rather than a target. That covers `https:`, `https://`, the opaque `https:centreon.example.com`, a credential with no host such as `https://user:pass@`, and a port-only authority such as `https://:8443`. The port-only form is the one that could previously start: Go's dialer treats an empty hostname as localhost, so `http://:8080` with `CENTREON_ALLOW_HTTP=true`, or `https://:8443` with `CENTREON_ALLOW_SELF_SIGNED=true`, did work. Write the name out instead (`http://localhost:8080`); note this also changes the `Host` header from `:8080` to `localhost:8080`, which matters if a name-based reverse proxy sits in front of Centreon.

> **Note on read-only mode:** set `MCP_READ_ONLY=true` to expose the read tools normally while every tool whose description ends with `Writes to Centreon.` is still listed but refuses with a tool-level error and performs nothing. That set, which the tool annotations mark as non-read-only, covers every state-changing tool: acknowledgements, downtimes, comments, forced checks, submitted results, the create, update, and delete tools for hosts, services, users, time periods, and filters, and `centreon_poller_apply`/`centreon_poller_apply_all`. This is the safe default to recommend for agent-driven or exploratory use, and it matters most in HTTP env-auth mode, where one shared pre-authenticated client otherwise backs the full write surface for anyone who can reach the port. `centreon_resource_submit` and `centreon_resource_check` are classified as destructive (they overwrite or force-refresh a resource's live status), so they are refused in read-only mode too and carry a destructive annotation for clients that key their own guardrails off it.

> **Note on untrusted tool data:** read tools return free text that comes from the monitored systems and their operators (plugin and check output, host and service names and aliases, notes, acknowledgement and downtime comments, and custom macro values). A compromised or hostile monitored host controls its own check output, so that text is attacker-influenced. Every read result wraps its payload between `CENTREON_DATA_BEGIN` and `CENTREON_DATA_END` markers, and the server Instructions tell the model to treat everything inside those markers strictly as data, never as instructions. The markers cannot be spoofed by field content, because the JSON serializer escapes the angle brackets the markers are built from. This is a defense-in-depth measure against prompt injection, best paired with `MCP_READ_ONLY=true` for untrusted or agent-driven use.

> **Note on the reported host:** `centreon_connection_test` and `centreon_platform_status` name the Centreon host they are talking to, with all credentials stripped. When the configured URL carries an `@` after the authority, whether written literally or percent-encoded, the server reports `(redacted host)` instead of a name. The same happens for a percent sign that is itself encoded, as in `/100%25`, because decoding it leaves a `%` that cannot be resolved further and so cannot be ruled out as an encoded `@`. Ordinary escapes such as `%20` are unaffected. A URL of the form `scheme://a/b@c` cannot be told apart from a credential `a/b` plus a host `c` without resolving names, so the server refuses to echo a host it cannot vouch for rather than risk printing part of a password. Both rules apply only to a non-bracketed authority: a bracketed IPv6 host such as `https://[::1]:8443/path@x` still reports its name, because a bracketed authority has to parse as an IP address and so cannot be a mis-read credential. To get the name back, remove the `@` from the path, query or fragment; percent-encoding it does not help, because an encoded delimiter is exactly the case the check is guarding against. Everything else about the connection is unaffected: only the reported name changes.
...
Enable it with `AUTH_MODE=gateway` alongside `MCP_TRANSPORT=http`. The effective Centreon host and credentials arrive per request via HTTP headers. `CENTREON_HOST` and a startup credential (`CENTREON_TOKEN`, or `CENTREON_USERNAME` + `CENTREON_PASSWORD`) are still required by `LoadConfig` but act only as placeholders. `CENTREON_HOST` is not validated at all in gateway mode, neither its scheme nor whether it names a host; each `X-Centreon-Host` is validated per request instead.
...
|-----------------------------|-------------------------|-------|-----------------------------------------------------------------------------------|
| `monitoring_tools.go`       | Real-time monitoring    | 12    | `centreon_monitoring_host_list`, `centreon_monitoring_service_list`, `centreon_monitoring_resource_list` |
...
| `service_config_tools.go`   | Service configuration   | 17    | `centreon_service_get`, `centreon_service_create`, `centreon_service_template_list` |
