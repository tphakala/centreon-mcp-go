...
> **Note on transport security:** to keep credentials off the wire, the server rejects a cleartext `http://` `CENTREON_HOST` (except in gateway mode, where it is an unused placeholder), `CENTREON_ALLOWED_HOSTS` entry, or gateway `X-Centreon-Host` value (CWE-319); use `https://`. For a trusted LAN or loopback deployment you can opt back in with `CENTREON_ALLOW_HTTP=true`, but credentials then travel unencrypted. Upgrading an existing `http://` deployment requires setting this flag; every `CENTREON_ALLOWED_HOSTS` entry must include a scheme (`https://`, or `http://` with the flag set). A URL with no hostname is rejected everywhere a host is validated, since nothing can be dialled from it: that covers `https:`, `https://`, the opaque `https:centreon.example.com`, and a credential with no host such as `https://user:pass@`.
