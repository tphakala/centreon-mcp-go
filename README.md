...
> **Note on the reported host:** `centreon_connection_test` and `centreon_platform_status` name the Centreon host they are talking to, with all credentials stripped. When the configured URL carries an `@` after the authority, the server reports `(redacted host)` instead of a name. A URL of the form `scheme://a/b@c` cannot be told apart from a credential `a/b` plus a host `c` without resolving names, so the server refuses to echo a host it cannot vouch for rather than risk printing part of a password. If you see this, percent-encode the `@` in the path or query, or remove it.
